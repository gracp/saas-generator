...
import { createRepo, createIssue, createWorktree, gh, type RepoInfo } from './github';
...
export async function initializeProject(
  ...
  dbCreateProject({ name: opts.projectName, userId: opts.userId }).catch((err) =>
    console.error('[DB] Failed to create project in DB:', err)
  );
...
export async function researchAndGenerateIdeas(
  ...
  transitionStatus(projectId, 'generating_ideas', 'Generating SaaS ideas from research...', 'info');
  ...
  transitionStatus(projectId, 'selecting', 'Ideas ready — awaiting user selection', 'info');
...
export async function deployProject(
  ...
  try {
    await dbConnect();
    await dbUpdateProject(projectId, { vercelUrl: deployedUrl, status: 'live' });
  } catch {}
