...
function logWebhook(
  type: string,
  status: 'processed' | 'duplicate' | 'error' | 'verified_failed',
  detail?: string
) {
...
export async function POST(request: Request) {
  ...
  try {
        ...
        if (userEmail && customerId) {
          try {
            ...
            logWebhook(
              event.type,
              'error',
              `checkout.completed_failed: ${err instanceof Error ? err.message : 'unknown'}`
            );
          }
        }
        break;
      ...
      case 'customer.subscription.deleted': {
        ...
        try {
          ...
          logWebhook(
            event.type,
            'error',
            `subscription_deleted_failed: ${err instanceof Error ? err.message : 'unknown'}`
          );
        }
        break;
      }
    ...
    logWebhook(
      event.type,
      'error',
      `handler_failed: ${err instanceof Error ? err.message : 'unknown'}`
    );
  }
